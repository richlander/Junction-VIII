    ...
    public class GeneralSettingsViewModel : ViewModelBase
    {
        ...
        public static void AutoDetectSystemPaths(Settings settings)
        {
            ...
            else
            {
                ...
                {
                    string ff8path = Path.GetDirectoryName(settings.FF8Exe);
                    ...
                    if (
                        ...
                        (
                            (Directory.Exists(Path.Combine(ff8path, "music/vgmstream")) && Directory.EnumerateFiles(Path.Combine(ff8path, "music/vgmstream"), "*.ogg").Any()) || // did it inherit the original soundtrack?
                            Path.Exists(Path.Combine(ff8path, "ff8_en.exe")) || // did it inherit the original Steam exe?
                            Path.Exists(Path.Combine(ff8path, "firewall_entry.vdf")) || // did it inherit misc Steam files?
                            Path.Exists(Path.Combine(ff8path, "AF3DN.P")) // did it inherit offical Steam driver?
                        )
                    )
                    {
                        // Clearly not a genuine 2000 installation, try to convert it to Steam
                        ...
                        {
                            MessageDialogWindow.Show("Looks like you're using a previously converted Steam to 2000 Game Copy. It is suggested to install a fresh copy of the game from Steam and relaunch this app. The application will now close.", "This installation is now unsupported", System.Windows.MessageBoxButton.OK, System.Windows.MessageBoxImage.Error);
        ...
        private static bool AssociateIroFilesWithJ8()
        {
            try
            {
                ...
                RegistryHelper.SetValueIfChanged("HKEY_CLASSES_ROOT\\.iroj", "", $"JunctionVIII Mod");
        ...
        private static bool RemoveIroFileAssociationFromRegistry()
        {
            try
            {
                RegistryHelper.DeleteKey("HKEY_CLASSES_ROOT\\JunctionVIII\\shell\\Unpack IRO\\command");
                RegistryHelper.DeleteKey("HKEY_CLASSES_ROOT\\JunctionVIII\\DefaultIcon");
        ...
        private static bool AssociateIrosUrlWithJ8()
        {
            try
            {
                RegistryHelper.SetValueIfChanged("HKEY_CLASSES_ROOT\\iros", "", $"J8 Catalog Subscription");
                RegistryHelper.SetValueIfChanged("HKEY_CLASSES_ROOT\\iros", "URL Protocol", $"");
                RegistryHelper.SetValueIfChanged("HKEY_CLASSES_ROOT\\iros\\DefaultIcon", "", $"\"{Sys._J8Exe}\"");
                RegistryHelper.SetValueIfChanged("HKEY_CLASSES_ROOT\\iros\\shell\\open\\command", "", $"\"{Sys._J8Exe}\" \"%1\"");
        ...
        private static bool AssociateFileExplorerContextMenuWithJ8()
        {
            try
            {
                ...
                RegistryHelper.SetValueIfChanged("HKEY_CLASSES_ROOT\\Directory\\shell\\Pack into IRO", "Icon", $"\"{Sys._J8Exe}\"");
                RegistryHelper.SetValueIfChanged("HKEY_CLASSES_ROOT\\Directory\\shell\\Pack into IRO\\command", "", $"\"{Sys._J8Exe}\" /PACKIRO:\"%1\"");

                // create registry keys for 'Unpack IRO' for files
                RegistryHelper.SetValueIfChanged("HKEY_CLASSES_ROOT\\JunctionVIII\\shell\\Unpack IRO", "Icon", $"\"{Sys._J8Exe}\"");
                RegistryHelper.SetValueIfChanged("HKEY_CLASSES_ROOT\\JunctionVIII\\shell\\Unpack IRO\\command", "", $"\"{Sys._J8Exe}\" /UNPACKIRO:\"%1\"");
        ...
        private static bool RemoveFileExplorerContextMenuAssociationWithJ8()
        {
            try
            {
                ...
                RegistryHelper.DeleteKey("HKEY_CLASSES_ROOT\\JunctionVIII\\shell\\Unpack IRO");
