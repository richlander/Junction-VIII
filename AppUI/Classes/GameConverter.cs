...
namespace AppUI.Classes
{
    ...
    public class GameConverter
    {
        ...
        public bool IsGamePirated()
        {
            ...
            if (Sys.Settings.FF8InstalledVersion == FF8Version.Steam)
            {
                byte[][] requiredHashes = {
                    ...
                    Convert.FromHexString("269C12BC1E4A127F60ABFF47A525F87B503DE615"), // Steam 4GB
                };
        ...
        internal bool BackupExe(string backupFolderPath)
        {
            ...
            string ff8ExePath = Path.Combine(InstallPath, "ff8.exe");
        ...
        public void MoveOriginalAppFilesToBackup(string pathToBackup)
        {
            ...
            List<string> filesToMove = new List<string>() { "app.log", "ff8.exe", "ff8config.exe", "RunFFVIIConfig.bat", "RunFFVIIConfig.exe", "ff8_mo.exe", "ff8_nt.exe", "ff8_ss.exe", "ff8_ss_safer.exe", "ff8_bc.exe", "ff8input.cfg", "Multi_Readme.txt", "cfg.log", "Hext.log", "FF8_GC.log", "eax.dll", "Hext.dll", "multi.dll", "ff8_opengl.cfg", "ff8_opengl.pdb", "ff8_opengl.reg", "crash.dmp", "ff8_opengl.fgd", @"\plugins\ff8music.fgp", @"\plugins\ffmpeg_movies.fgp", @"\plugins\vgmstream_music.fgp" };
            ...
            foreach (string file in expectedFiles)
            {
                ...
                foreach (string label in volumeLabels)
                {
                    ...
                    if (!string.IsNullOrWhiteSpace(driveLetter))
                    {
                        ...
                        {
                            fullSourcePath = Path.Combine(driveLetter, "data", file); // ff8install disc has a different path then disc1-3
                        }
